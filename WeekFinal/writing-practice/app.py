...
class SpanishLearningApp:
    def __init__(self):
        logger.debug("Initializing Spanish Learning App...")
    ...
    def generate_sentence(self, word: dict) -> str:
        """Generate a sentence using a simple template system"""
        try:
            # Simple template-based sentence generation
            templates = [
                "Me gusta {verb} {object}.",
                "{timeframe} voy a {verb} {object}.",
                "Yo {verb} {object} {timeframe}.",
            ]
            
            verbs = ["comer", "ver", "usar", "tener"]
            timeframes = ["hoy", "mañana", "cada día"]
            
            template = random.choice(templates)
            sentence = template.format(
                verb=random.choice(verbs),
                object=word.get('spanish', 'algo'),
            ...
            english = sentence
            for spanish, english_word in translations.items():
                english = english.replace(spanish, english_word)
            
            return f"Spanish: {sentence}\nEnglish: {english}"
    ...
    def grade_submission(self, image) -> Dict:
        ...
        return {
            "transcription": "Hoy voy a comer tacos",
            "translation": "Today I am going to eat tacos",
            "grade": "S",
    ...
    def render_setup_state(self):
        ...
        if generate_button:
            ...
            logger.debug(f"Selected word: {word.get('english')} - {word.get('spanish')}")
    ...
    def render_practice_state(self):
        ...
        st.title("Practice Spanish")
        st.write(f"English Sentence: {st.session_state.current_sentence}")
        
        uploaded_file = st.file_uploader("Upload your written Spanish", type=['png', 'jpg', 'jpeg'])
